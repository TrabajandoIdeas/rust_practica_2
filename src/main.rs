...
fn main() {
  ...
  println!("Duplicar valores [3.5, 2.9, 5.0]: {:?}", utils::duplicar_valores::<SIZE>(&[3.5, 2.9, 5.0]));
  println!("Cantidad mayores a 30 en [32, 23, 65, 2, 4, 40]: {}", utils::cantidad_de_mayores(&[32, 23, 65, 2, 4, 40], 30));
  println!("Sumar arreglos [23.5, 10.0] + [82.5, 87.9]: {:?}", utils::sumar_arreglos::<SIZE>(&[23.5, 10.0], &[82.5, 87.9]));
  println!("Cantidad en rango (15, 40) en [33, 43, 3]: {}", utils::cantidad_en_rango(&[33, 43, 3], 15, 40));
  println!("Longitud de cadenas {:?}", utils::longitud_de_cadenas::<3>(&[String::from("Hola"), String::from(" "), String::from("Mundo")]));
}
